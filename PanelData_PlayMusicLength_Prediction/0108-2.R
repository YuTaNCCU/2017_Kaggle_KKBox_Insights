...
filter_1_ul_c <- fread(file = 'Data/filter_1_ul_c.csv', sep = ",", header=T, stringsAsFactors = T)
filter_1_mb_c <- fread(file = 'Data/filter_1_mb_c.csv', sep = ",", header=T, stringsAsFactors = T)
filter_1_ts <- fread(file = 'Data/filter_1_ts.csv', sep = ",", header=T, stringsAsFactors = T)
...
#create single period features function
create_feature <- function(period_i){
  #Y
  Y_temp_ts  <- filter_1_ts %>%
    filter(period_name[[period_i]][4] <= transaction_date  & 
             transaction_date <= period_name[[period_i]][5] ) %>%
    group_by(msno) %>%
    summarise( sum_pay = round( sum(actual_amount_paid) ,2) )
  
  Y_temp_ul  <- filter_1_ul_c %>%
    filter(period_name[[period_i]][4] <= date  & 
             date <= period_name[[period_i]][5] ) %>%
    group_by(msno) %>%
    summarise( sum_play_secs = round( sum(total_secs) ,2) )
  
  Y_temp <-filter_1_mb_c %>%
  ...
  X_temp_ts <- filter_1_ts %>%
    filter(transaction_date <= period_name[[period_i]][5] ) %>%
    group_by(msno) %>%
    summarise(
      ...
      auto_renew = max(is_auto_renew) #大多有自動續約？
    )
  
  X_temp_ul <- filter_1_ul_c %>%
    filter(date <= period_name[[period_i]][5] ) %>%
    group_by(msno) %>%
    summarise(
  ...
  X_temp <- filter_1_mb_c %>%
    inner_join(X_temp_ts, by='msno') %>%  #確保每個會員都有 member 及 transction的資料
    left_join(X_temp_ul, by='msno')  %>%   #會員可以有userlog的空值
    mutate_all(funs(ifelse(is.na(.), 0, .)))  #NA補 0
  
  # Join Y & X
  YX <- cbind(period_i, period_name[[period_i]][1], period_name[[period_i]][2], period_name[[period_i]][3], Y_temp,X_temp)
  colnames(YX)[1:4] <- c('period_id', 'period_name', 'year', 'quarter')
  return( YX )
...
for(period_i in 1:9){
  ...
  filename <- paste( 'YX', period_i )
  assign( filename, create_feature(period_i) )
  get( filename ) %>% 
    fwrite( file = paste('Data/',filename,'.csv'), append = FALSE, quote = "auto" )
